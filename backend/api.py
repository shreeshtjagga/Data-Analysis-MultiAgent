...
from .auth import login_user, register_user, request_password_reset, reset_password_with_token, verify_access_token, refresh_session
...
from .core.chat_engine import answer_question, answer_chart_explanation
...
SYNTHESIS_MODEL = os.getenv('GROQ_SYNTHESIS_MODEL', os.getenv('GROQ_MODEL', 'openai/gpt-oss-120b'))
...
async def chat_with_analysis(body: ChatRequest, user_id: int=Depends(get_current_user_id), db: AsyncSession=Depends(get_db)):
    ...
    if file_hash:
        ...
        try:
            ...
            if _fh_row.scalar_one_or_none() is None and user_id != 0:

                _fh_any = await db.execute(_sa_select(_AnalysisHistory.id).where(_AnalysisHistory.file_hash == file_hash).limit(1))

                if _fh_any.scalar_one_or_none() is None:

                    logger.debug('file_hash %s not found in DB history; proceeding with context payload', file_hash[:8])

        except Exception as _fh_exc:

            logger.debug('file_hash check skipped (using context): %s', _fh_exc)
    ...
    if intent == 'generate_chart':
        ...
        from .agents.plot_generator import plan_and_generate_custom_chart

        chart_plan = await plan_and_generate_custom_chart(
        ...
        if chart_plan.get('cannot_plot'):

            return {

                'answer': chart_plan.get('reason', "Could not generate chart for this request."),
        ...
        chart_res = chart_plan.get('chart_result')

        if not chart_res or chart_res.get('error'):

            err_msg = chart_res.get('error') if chart_res else "Chart generation failed."

            return {'answer': err_msg, 'data_queried': False, 'new_chart': None}

        if chart_res.get('is_duplicate'):

            return {'answer': "This chart is already on your dashboard! If you'd like to see something else, specify different columns or filter conditions.", 'data_queried': False, 'new_chart': None}

        explanation = chart_plan.get('explanation') or "Here is the chart you requested."

        return {'answer': explanation, 'data_queried': False, 'new_chart': chart_res}
