...
def read_csv_with_fallback(file_bytes: bytes, max_analyze_rows: int, max_analyze_columns: int) -> pd.DataFrame:
    ...
    for csv_kwargs in attempts:
        try:
            ...
            if header_df.shape[1] > max_analyze_columns:
                raise HTTPException(status_code=413, detail=f'Dataset has {header_df.shape[1]} columns. Maximum allowed is {max_analyze_columns}.')
            return pd.read_csv(io.BytesIO(file_bytes), nrows=max_analyze_rows + 1, on_bad_lines='skip', **csv_kwargs)
