...
def _skew_to_english(skew: Any) -> str:
    try:
        ...
        if s > 2.0:
            return 'very strongly right-skewed — a small number of very high values pull the average up'
        if s > 1.0:
            return 'right-skewed — most values are low but some very high values exist'
        if s > 0.4:
            return 'slightly right-skewed'
        if s < -2.0:
            return 'very strongly left-skewed — a small number of very low values pull the average down'
        if s < -1.0:
...
def _build_overview(stats: dict, insights: dict) -> dict:
    ...
    text = f"This dataset is '{label}' in the {domain} domain. {description} It has {row_count:,} rows and {col_count} columns total. Numeric columns ({len(num_cols)}): {', '.join(num_cols[:25])}. Categorical columns ({len(cat_cols)}): {', '.join(cat_cols[:15])}. Data completeness: {completeness:.1f}%. Missing values: {missing:,} cells across the dataset. Duplicate rows: {dupes}. "
    if headline:
        text += f'Key finding: {headline}'
    return {'chunk_id': 'overview', 'chunk_type': 'overview', 'column': 'all', 'text': text.strip()}
...
def _build_data_quality(stats: dict) -> dict:
    ...
    worst_str = '; '.join((f"'{col}' has {pct:.1f}% missing ({missing_vals.get(col, '?')} rows)" for (col, pct) in worst))
    text = f'Data quality: {completeness:.1f}% complete. Total cells: {total:,}. Missing cells: {missing:,}. Duplicate rows: {dupes}. '
    if worst_str:
        text += f'Columns with missing data: {worst_str}.'
    return {'chunk_id': 'data_quality', 'chunk_type': 'data_quality', 'column': 'all', 'text': text.strip()}
...
def _build_column_stat(col: str, d: dict, missing_pct: float, row_count: int) -> dict:
    ...
    text = f"Column '{col}' numeric statistics: average = {_f(d.get('mean'))}, median = {_f(d.get('median'))}, standard deviation = {_f(d.get('std'))}, minimum = {_f(d.get('min'))}, maximum = {_f(d.get('max'))}, 25th percentile = {_f(d.get('q1'))}, 75th percentile = {_f(d.get('q3'))}, IQR = {_f(d.get('iqr'))}. Non-null count: {d.get('count', 'N/A')}. Missing: {null_rows} rows ({missing_pct:.1f}%)."
    return {'chunk_id': f'col_stat_{col}', 'chunk_type': 'column_stat', 'column': col, 'text': text.strip()}

def _build_distribution(col: str, d: dict) -> dict:
    shape = _skew_to_english(d.get('skewness'))
    text = f"Distribution of '{col}': {shape}. Values range from {_f(d.get('min'))} to {_f(d.get('max'))}. Average is {_f(d.get('mean'))}, median is {_f(d.get('median'))}. The middle 50% of values fall between {_f(d.get('q1'))} and {_f(d.get('q3'))}. Skewness = {_f(d.get('skewness'), 3)}, kurtosis = {_f(d.get('kurtosis'), 3)}."
    return {'chunk_id': f'dist_{col}', 'chunk_type': 'distribution', 'column': col, 'text': text.strip()}

def _build_categorical(col: str, d: dict, row_count: int) -> dict:
    rc = max(row_count, 1)
    top5 = d.get('top_5_values') or {}
    top_str = ', '.join((f"'{k}' = {v} times ({v / rc * 100:.1f}%)" for (k, v) in list(top5.items())[:5]))
    ...
    text = f"Categorical column '{col}': {d.get('unique_values', 0)} unique values across {row_count:,} rows (diversity ratio = {diversity:.3f}). Most common: '{most_common}' appears {most_count:,} times = {most_pct:.1f}% of all rows. Least common: '{least_common}' appears {least_count} times. Top 5 values: {top_str}."
    return {'chunk_id': f'cat_{col}', 'chunk_type': 'categorical_freq', 'column': col, 'text': text.strip()}
...
def _build_correlation(c1: str, c2: str, r: float) -> dict:
    ...
    if r_abs >= 0.9:
        ...
        strength = 'weak'
    trend = f"When '{c1}' increases, '{c2}' tends to {('increase' if r > 0 else 'decrease')} as well."
    text = f"Correlation between '{c1}' and '{c2}': r = {r:.4f}. This is a {strength} {direction} linear relationship. {trend}"
    return {'chunk_id': f'corr_{c1}_{c2}', 'chunk_type': 'correlation', 'column': f'{c1},{c2}', 'text': text.strip()}
...
def _build_outlier(col: str, d: dict, row_count: int) -> dict:
    ...
    text = f"Outliers in '{col}': {count} rows ({pct:.2f}% of {row_count:,} total rows) have values outside the normal range of {_f(lo)} to {_f(hi)}. These extreme values may represent data errors, rare events, or genuinely exceptional cases worth investigating."
    return {'chunk_id': f'outlier_{col}', 'chunk_type': 'outlier', 'column': col, 'text': text.strip()}

def _build_finding(idx: int, finding: str) -> dict:
    return {'chunk_id': f'finding_{idx}', 'chunk_type': 'insight_finding', 'column': 'all', 'text': f'Key finding #{idx + 1}: {finding}'}

def _build_recommendation(idx: int, rec: str) -> dict:
    return {'chunk_id': f'rec_{idx}', 'chunk_type': 'recommendation', 'column': 'all', 'text': f'Actionable recommendation: {rec}'}
...
def _build_chart_chunk(key: str, fig_data: Any) -> Optional[dict]:
    try:
        ...
        title = (title_raw.get('text') if isinstance(title_raw, dict) else title_raw) or key
        ...
        for trace in (fig.get('data') or [])[:3]:
            ...
            if labels and values:
                has_data = True
                pairs = [f"'{lbl}': {_f(val)}" for (lbl, val) in zip(labels[:12], values[:12])]
                ...
                if numeric_y:
                    ...
                    facts.append(f"Type={ttype}. Highest: '{x_at_max}' = {_f(max_v)}. Lowest: '{x_at_min}' = {_f(min_v)}. Average: {_f(avg_v)}. All X: {x_vals[:20]}. All Y: {[_f(v) for (_, v) in numeric_y[:20]]}.")
        if not has_data:
            return None
        return {'chunk_id': f'chart_{key}', 'chunk_type': 'chart_summary', 'column': 'all', 'text': ' '.join(facts).strip()}
    except Exception as exc:
        logger.warning("Chart chunk build failed for '%s': %s", key, exc)
        return None

async def build_rag_index(file_hash: str, stats: dict, insights: dict, charts: dict, redis_client) -> int:
    import asyncio
    ...
    try:
        texts_backup = [{'chunk_id': c['chunk_id'], 'chunk_type': c['chunk_type'], 'column': c['column'], 'text': c['text']} for c in raw_chunks]
        payload = json.dumps(texts_backup, ensure_ascii=True)
        await redis_client.setex(_REDIS_CHUNKS_KEY.format(file_hash), _REDIS_TTL, payload)
        logger.info('RAG index complete: %d chunks, namespace=%s, redis_backup=ok', count, file_hash[:8])
    except Exception as exc:
        logger.warning('RAG Redis backup failed (non-fatal): %s', exc)
    return count

async def retrieve_chunks(file_hash: str, question: str, k: int=8, redis_client=None) -> list[dict]:
    import asyncio
    ...
    if q_vec is not None:
        ...
        if results:
            logger.info('RAG retrieved %d chunks via Pinecone (top score: %.3f)', len(results), results[0].get('score', 0))
            return results
