...
_LOCAL_SQLITE_PATH = os.path.join(_STORAGE_DIR, 'datapulse.db').replace('\\', '/')

_LOCAL_SQLITE_URL = f'sqlite+aiosqlite:///{_LOCAL_SQLITE_PATH}'

DATABASE_URL = os.getenv('DATABASE_URL', '')

if os.getenv('USE_LOCAL_DB', 'false').lower() == 'true' or not DATABASE_URL or 'sqlite' in DATABASE_URL:

    DATABASE_URL = _LOCAL_SQLITE_URL

    logger.info('Using local SQLite database: %s', DATABASE_URL)
...
class AnalysisHistory(Base):
    ...
    user_id = Column(_PK_TYPE, ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
...
class AnalysisMetadata(Base):
    ...
    analysis_id = Column(_PK_TYPE, ForeignKey('analysis_history.id', ondelete='CASCADE'), nullable=False, unique=True, index=True)
