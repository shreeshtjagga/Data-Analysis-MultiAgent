    ...
    try:
        ...
        for col in df.select_dtypes(include=['object']).columns:
            ...
            if total > 0 and valid / total > 0.5:
                df[col] = converted
                coerced_columns.append({'column': col, 'valid_ratio': round(valid / total, 3)})
                logger.info("Coerced '%s' from object to numeric (%.0f%% valid)", col, valid / total * 100)
        stats_summary: dict = {}
        ...
        stats_summary['missing_values'] = {col: int(count) for (col, count) in missing_data.items() if count > 0}
        stats_summary['missing_percentage'] = {col: float(count / len(df) * 100) for (col, count) in missing_data.items() if count > 0}
        numeric_cols = [c for c in df.select_dtypes(include=[np.number]).columns.tolist() if c not in excluded_names]
        numeric_stats: dict = {}
        ...
        for col in numeric_cols:
            try:
                ...
                iqr = q3 - q1
                numeric_stats[col] = {'mean': float(col_data.mean()), 'median': float(col_data.median()), 'std': float(col_data.std()), 'min': float(col_data.min()), 'max': float(col_data.max()), 'q1': q1, 'q3': q3, 'iqr': iqr, 'skewness': float(scipy_stats.skew(col_data, nan_policy='omit')), 'kurtosis': float(scipy_stats.kurtosis(col_data, nan_policy='omit')), 'variance': float(col_data.var()), 'count': int(col_data.count())}
                ...
                if len(outliers) > 0:
                    outliers_summary[col] = {'count': int(len(outliers)), 'percentage': float(len(outliers) / len(df) * 100), 'lower_bound': float(lower_bound), 'upper_bound': float(upper_bound)}
        ...
        categorical_cols = [c for c in df.select_dtypes(include=['object']).columns.tolist() if c not in excluded_names]
        ...
        for col in categorical_cols:
            try:
                ...
                categorical_stats[col] = {'unique_values': int(df[col].nunique()), 'most_common': _sanitize_cell_for_output(value_counts.index[0]) if len(value_counts) > 0 else None, 'most_common_count': int(value_counts.iloc[0]) if len(value_counts) > 0 else 0, 'least_common': _sanitize_cell_for_output(value_counts.index[-1]) if len(value_counts) > 0 else None, 'least_common_count': int(value_counts.iloc[-1]) if len(value_counts) > 0 else 0, 'diversity_ratio': float(df[col].nunique() / len(df)), 'top_5_values': top_5_values}
            except Exception as col_err:
        ...
        if len(numeric_cols) > 1:
            try:
                corr_cols = [c for c in numeric_cols[:50] if df[c].notna().sum() / max(len(df), 1) >= 0.5]
                if len(corr_cols) < 2:
                    stats_summary['strong_correlations'] = []
                else:
                    corr_sample = df[corr_cols].sample(min(len(df), 5000), random_state=42) if len(df) > 5000 else df[corr_cols]
                    ...
                    for (i, c1) in enumerate(cols_list):
                        for (j, c2) in enumerate(cols_list):
                            ...
                            if abs(corr_val) > 0.7 and pd.notna(corr_val):
                                strong_correlations.append({'col1': c1, 'col2': c2, 'correlation': float(corr_val)})
                    strong_correlations = sorted(strong_correlations, key=lambda item: abs(item['correlation']), reverse=True)[:_MAX_STRONG_CORRELATIONS]
            ...
            stats_summary['strong_correlations'] = []
        stats_summary['data_quality'] = {'total_cells': int(len(df) * len(df.columns)), 'missing_cells': total_missing, 'duplicate_rows': int(len(df) - len(df.drop_duplicates())), 'completeness': float((len(df) * len(df.columns) - total_missing) / max(len(df) * len(df.columns), 1) * 100)}
        ...
        state.clean_df = df
        logger.info('Statistician complete. %d numeric, %d categorical columns analysed', len(numeric_stats), len(categorical_stats))
        state.completed_agents.append('statistician')
    except Exception as e:
        logger.error('Statistician error: %s', e)
        add_pipeline_error(state.errors, code='STATISTICIAN_FAILED', message=str(e), agent='statistician', error_type='agent')
    return state